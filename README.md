...
2. Modify the `nuscenes_path` and `nuscenes_version` in [nuscenes.yaml](config/nuscenes.yaml);
3. run offline mapping by following command:
```
catkin_make
source devel/setup.bash
roslaunch fast_lio mapping_nuscenes_offline.launch
```
